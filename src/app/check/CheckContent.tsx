...
export default function CheckContent() {
  ...
  const [showArticles, setShowArticles] = useState<{ [key: number]: boolean }>({})
  ...
  const relatedLaws = [
    ...
    },
    {
      ...
      content:
        '２　酒類に炭酸ガスを加える場合の取扱い　酒類に炭酸ガス（炭酸水を含む。3において同じ。）を加える行為は、新たな製造行為となる。したがって、酒類に炭酸ガスを加える場合は、当該酒類の製造免許を有する場合に限られる。',
    },
    {
      article: '酒税法 第43条（みなし製造）',
      content:
  ...
  return (
    <div className={`${inter.className} ${mplus.className} min-h-screen bg-sky-50 pt-20`}>
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="bg-white shadow p-4 rounded space-y-2 text-slate-700">
        ...
        <div className="bg-white shadow p-4 rounded">
          <h2 className="text-lg font-bold text-slate-700 mb-2">リスク度合い</h2>
          <p className="text-red-700 font-bold text-3xl">リスク大</p>
          <p className="text-slate-700 pl-10 mt-2">
        ...
        <div className="bg-white shadow p-4 rounded">
          <div className="flex items-center space-x-4 mb-2">
            <h2 className="text-lg font-bold text-slate-700">関連法令</h2>
            <button
              onClick={() => {
                const allOpen = Object.keys(showArticles).length === relatedLaws.length &&
                                Object.values(showArticles).every(v => v)
              ...
              className="text-sm text-blue-600 hover:underline border border-slate-800 px-2 py-1 rounded"
            >
              {Object.keys(showArticles).length === relatedLaws.length &&
               Object.values(showArticles).every(v => v)
                ? 'すべて閉じる'
                : '条文をすべて開く'}
            </button>
          ...
          <ul className="space-y-2 pl-10">
            {relatedLaws.map((law, index) => (
              <li key={index} className="border-b pb-2">
                <p className="font-semibold text-slate-800">{law.article}</p>
                <button
                  onClick={() => setShowArticles(prev => ({ ...prev, [index]: !prev[index] }))}
                  className="text-sm text-blue-600 hover:underline border border-slate-800 px-2 py-1 mt-1"
                >
                  条文表示
                </button>
                {showArticles[index] && <p className="mt-1 text-slate-700">{law.content}</p>}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bg-white shadow p-4 rounded">
          <h2 className="text-lg font-bold text-slate-700 mb-2">次のアクションの提案</h2>
          <p className="text-slate-700 whitespace-pre-line pl-10">
        ...
        <div className="flex justify-center space-x-4">
          <button className="px-4 py-2 bg-sky-700 hover:bg-sky-800 text-white rounded">
            リスク程度を理解して施策を実施
          </button>
          <button
        ...
        <div className="mt-2 flex justify-end">
          <button
            onClick={() => router.push('/')}
            className="mt-4 px-6 py-2 bg-gray-600 text-white rounded hover:bg-slate-700"
          >
            トップに戻る
          </button>
        </div>
